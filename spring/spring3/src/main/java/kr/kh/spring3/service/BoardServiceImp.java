...
public class BoardServiceImp implements BoardService{
	...
	private void uploadFile(int bo_num, MultipartFile file) {
		...
		//파일경로, 파일이름, 실제 파일
		boardDao.insertFile(fileVO); //DB에 첨부파일 정보를 추가하는 코드
		}catch(Exception e) {
			e.printStackTrace();
		}
	}//예외 발생 할 수 있으므로 try catch로 감싸기
	...
	public boolean insertBoard(BoardVO board, MemberVO user, MultipartFile[] files) {
		...
		if(!res) 
			return false;
		//게시글 등록에 실패하면 첨부파일 등록할 필요 업승
		if(files == null || files.length == 0)
			return true;
		
		for(MultipartFile file : files) { uploadFile(board.getBo_num(),file); //향상된
		  }
