...
<body>
<form action="<c:url value="/board/update" />" method="get" enctype="multipart/form-data" >
	<h1>게시글 수정</h1>
	...
	<div class="from-group">
		    ...
		    <select class="form-control" name="bo_co_num">
		    	<c:forEach items="${list}" var="co">
		    		<option value="${co.co_num }"
		    			<c:if test ="${co.co_num == board.bo_co_num}">selected
		    		 </c:if>>${co.co_name}</option>
		    	</c:forEach>
		    </select>
	  	</div>
		<div class="form-group">
	  	...
	  	<div class="form-group">
		    ...
		    <textarea rows="10" class="form-control" id="bo_content" name="bo_content">${board.bo_content }</textarea>
	  	</div>
...
$('[name = bo_content]').summernote({
  ...
  height: 400
	...
	$(".btn-del").click(function(){
		...
		//input file을 추가
		$(this).parnets(".box-attachment").append(`<input type="file" class="form-control" name="file">`);				
		//클릭한 x버튼의 첨부파일을 삭제(부모버튼 지우기)
		$(this).parent().remove();
		});
		</script>
	</body>
