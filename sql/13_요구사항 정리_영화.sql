...
- 조회 (무비차트)
    ...
    - 영화는 오늘을 기준으로 오늘 이후에 일정들을 예매한 내역을 기준으로 조회한다.
...
- 조회(상영시간)
    ...
    - 사용자가 영화, 극장, 날짜를 선택하면 영화관 정보( 영화관 이름, 전체 좌석수)와 상영시간 별 현재
	예매 가능 좌석수를 조회할 수 있다.
- 조회(영화 상세 조회)
	- 사용자는 영화 상세 정보를 조회할 수 있다.
    - 상세정보는 감독, 배우, 장르, 기본정보, 개봉일,
		트레일러, 스틸컷이 있다.
	- 장르는 등록된 장르를 보여주며, 장르는 여러가지일 수 있다.
    - 기본 정보에는 관람 나이정보, 러닝 시간, 제작국가가 있다.
    - 제작 국가는 여러 국가가 될 수 있다.
    - 감독은 여러 감독이 올 수 있다.
    - 배우는 주연, 조연 구분 없이 등록된 배우를 영화에 등록된 배우 순으로 조회한다. 
- 검색
	- 사용자는 검색을 통해 등록된 영화를 조회할 수 있다.
    - 조획되는 영화는 현재 상영과 상관없이 제목을 기준으로 조회한다.
회원
- 예매 
	- 예매는 회원만 가능하다.
...
관리자 기능
- 감독/배우 등록
	- 관리자는 감독/ 배우를 등록/수정/삭제 할 수 있다.
    - 감독/배우는 등록된 인물을 선택한 후, 역할을 선택하여 등록한다. 
