...
SELECT PR_CA_NUM AS '카테고리번호', COUNT(PR_CA_NUM) AS '제품수'
FROM PRODUCT group by PA_CA_NUM HAVING COUNT(PR_CA_NUM) >=2;

# ABC123 회원이 지금까지 주문한 총 금액을 조회하는 쿼리
SELECT OR_ME_ID AS '구매아이디', SUM(OR_TOTAL_PRICE) AS `총 구매금액` FROM `ORDER`
...
JOIN
	CATEGORY
ON
	# PRODUCT의 외래키를 CATEGORY의 기본키와 연걸
...
# 회원별 등급을 조회하는 쿼리, 등급은 기본이 브론즈, 누적금액이 5만원 이상이면 실버, 누적 금액이 8만원 이상이면 골드
# CASE문을 활용
SELECT 
ME_ID AS '아이디', 
CASE
