...
- DB명.테이블명은 현재 선택된 DB와 상관없이 작업할 수 있다(안정적)
*/
...
# 날짜형태인 문자열은 DATETIME에 저장하려 하면 변환이 가능하기 때문에 정상 동작
INSERT INTO MEMBER
...
UPDATE MEMBER member
    ...
    WHERE
		ID = "ID3";
# 아이디가 ID1인 회원의 비번을 IDABC로, 이메일을 IDABC@NAVER.COM으로 수정하는쿼리
UPDATE 
	MEMBER
SET
	PW = "IDABC",
...
# MEMBER 테이블을 조회 CTRL + ENTER
SELECT ID, PW, EMAIL, REG_DATE FROM MEMBER;
SELECT * FROM MEMBER;

# ID가 ID1인 회원의 정보를 조회
SELECT * FROM MEMBER COUNT(CO_ST_NUM) AS "수강생 수" WHERE ID = "ID1";
