...
//하나의 앱으로 여러 페이지를 처리할 때 사용하는 것 : BrowserRouter
...
function App() {
  ...
  return (
    <BrowserRouter>
      <Nav/>
      <Routes>
        <Route path="/" exact element={<Add list ={list} add={add} remove={remove}/>} />
  ...
  function addMusic(){
    ...
    return(
      <div>
        ...
        <div>
          <label>음악번호</label>
          <input type="text" onChange={numChange}/>
        </div>
        <div>
          <label>가수</label>
        ...
        <button onClick={addMusic}>음악추가</button>
      </div>
